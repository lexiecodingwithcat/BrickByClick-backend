...
async def create_project(project: ProjectTaskCreate, db:db_dependence, current_user:Annotated[User, Depends(get_current_user)]):
    # check admin
    if not current_user.is_admin:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,detail = "User is not authorized."
...
def add_task(id:int, db:db_dependence, task: TaskCreate):
    ...
    if db_task is None:
        ...
        existing_link = db.query(ProjectTask).filter(
            ProjectTask.project_id == id,
            ProjectTask.task_id == db_task.id
        ).first()
        if existing_link:
            raise HTTPException(
    ...
    db.refresh(db_project)
    db_tasks = db.query(Task).join(ProjectTask).filter(ProjectTask.project_id == id).all()
